...
def holm_bonferroni_correction(pvals,alpha):
    ...
    #Calculate sequentially smaller alpha values for each successive test
...
def split_to_contiguous(t_arr,x_arr=None,dt=None,max_t=None):
    ...
    return np.split(x_arr[1:],np.squeeze(np.argwhere(np.invert(is_contiguous(t_arr,dt,max_t)))))
